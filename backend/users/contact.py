...
def send_contact_message(request):
    ...
    try:
        ...
        if not all([name, email, message]):
            logger.warning(
                f"Contact form missing required fields: name={bool(name)}, email={bool(email)}, message={bool(message)}"
            )
            return Response(
        ...
        try:
            ...
            logger.error(
                f"Exception in send_contact_form_email: {str(email_error)}",
                exc_info=True,
            )
        ...
        logger.error(
            f"Unhandled exception in send_contact_message: {str(e)}", exc_info=True
        )
        return Response(
            {"message": "Your message has been received. We will contact you shortly."},
            status=status.HTTP_200_OK,
